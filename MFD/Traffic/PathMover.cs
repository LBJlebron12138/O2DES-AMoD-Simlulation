...
namespace O2DESNet.Traffic
{
    public class PathMover : Component<PathMover.Statics>
    {
        ...
        public class Statics : Scenario
        {
            ...
            public Path.Statics CreatePath(string tag = null, double length = 0, int capacity = int.MaxValue, ControlPoint.Statics start = null, ControlPoint.Statics end = null, bool crossHatched = false,double freespeed=0,int Lanes=0,int areatag=0)
            {
                CheckInitialized();
            ...
            private void ConstructRoutingTables()
            {
                ...
                if (RoutingTablesFile != null)
                {
                    ...
                    //try
                    // {
                    var linefile = File.ReadAllLines(RoutingTablesFile);
                    int i = 0;
            ...
            public void RoutingTableBuilder(string filename = "Troutingtable.txt")
            {
                List<Tuple<int, int, double>> edges = Paths.Select(link => new Tuple<int, int, double>(link.Value.Start.Index, link.Value.End.Index, link.Value.Length)).ToList();
                var floyd = new Floyd(edges);
                var points = ControlPoints.Values.OrderBy(cp => cp.Index).ToList();

                for (int i = 0; i < points.Count; i++)
                {
                    List<int> sinks = points.Select(p => p.Index).ToList();
                    int source = i;
                    sinks.Remove(source);
                    while (sinks.Count > 0)
                    {
                        
                        int sink = sinks.First();
                        List<int> path = floyd.ShortestPath(source, sink);
                        if (path.Count < 1)
                            throw new Exception();
                        else
                        {
                            ...
                            {
                                lock (ControlPoints)
                                    points[source].RoutingTable[points[path[j]]] = points[path[1]];
                                sinks.Remove(path[j]);
                            }
                        }
                    }
                ...
                string str = string.Join(";", points.Select(p => string.Format("{0}:{1}", p.Index,
                    string.Join(",", p.RoutingTable.Select(r => String.Format("{0}_{1}", r.Key, r.Value))))));
                sw.Write(str);
                sw.Close();
            }
            #endregion
        ...
        public double TotalEmptyMilage { get; private set; }
        #endregion
        ...
        private class DepartEvent : InternalEvent
        {
            ...
            public override void Invoke()
            {
                ...
                Execute(new ReachControlPointEvent { This = This, Vehicle = vehicle, At = Path.Config.Start });//To Beta_2
                This.HCounter_Departing.ObserveChange(-1, ClockTime);
        ...
        private class ReachControlPointEvent : InternalEvent
        {
            ...
            public override void Invoke()
            {
                if (At.Equals(Vehicle.Targets.First()))//When the vehicle j reaches point s
                {
                ...
                var path = This.Paths[At.PathTo(Vehicle.Targets.First())];
                Execute(path.Enter(Vehicle));//To Alpha(p)_1
            }
        }
        ...
        private class ArriveEvent : InternalEvent
        {
            ...
            public override void Invoke()
            {
                ...
                if (Vehicle.Idle == true)
                    Execute(This.OnArrive[0](Vehicle, At));
                else
                    Execute(This.OnArrive[1](Vehicle, At));
            }
        }
        ...
        //++
        private class CalcEmptyMilageEvent:InternalEvent
        {
            internal Path Path { get; set; }
            internal Vehicle Vehicle { get; set; }
            public override void Invoke()
            {
                if (Vehicle.Idle == true)
                {
                    This.TotalEmptyMilage += Path.Config.Length;
                    //This.TotalVehicleTime += ClockTime - This.Timestamps[Vehicle];
                    //This.Timestamps[Vehicle] = ClockTime;
                    Execute(Vehicle.CalcMilage(Path.Config.Length));
                }
        ...
        public PathMover(Statics config, int seed, string tag = null) : base(config, seed, tag)
        {
            ...
            foreach (var pathConfig in Config.Paths.Values)
            {
                ...
                path.OnExit.Add(veh => new ReachControlPointEvent { This = this, Vehicle = veh, At = path.Config.End });//(10)
                path.OnExit.Add(veh => new DepartEvent { This = this, Path = path });//(11)
                path.OnExit.Add(veh => new CalcEmptyMilageEvent { This = this, Path = path, Vehicle = veh });//++
            }
            foreach (var path in Paths.Values)
            {
